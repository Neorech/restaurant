<?php
    ...
    include'config.php';
		...
		//$sql=("UPDATE venta SET vFecha = '' WHERE vId =$id");
		$sql=("UPDATE venta SET vEliminado = vFecha , vFecha = '2014-01-01 00:00:00' WHERE vId =$id");
		...
		$sql=("UPDATE mesa SET mEstado = '0',  mPago = '0' WHERE mId =$estado");
		mysql_query($sql);
		
		$result=mysql_query("SELECT  vdId ,vId ,vd_idProducto ,vd_Cantidad ,vd_Costo FROM venta_detalle where vId= $id", $link);
...
//mysql_query("INSERT INTO venta_detalle(vId,vd_idProducto, vd_Cantidad, vd_Costo) VALUES ('$idultimo','".$valor['vdId']."','".$valor['vd_Cantidad']."','".$valor['vd_Costo']."')");
	...
	{ 
		...
		mysql_query("UPDATE almacen SET aCantidad = aCantidad + ".($rowalmacen["paCantidad"]*$valor['vd_Cantidad'])." where idAlmacen =".$rowalmacen["idAlmacen"]." ");
		}
